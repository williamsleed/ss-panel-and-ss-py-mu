...
install_ss_panel_mod_v3(){
	...
	cd default
	rm -rf index.html
	git clone https://github.com/williamsleed/mod.git tmp && mv tmp/.git . && rm -rf tmp && git reset --hard
...
Libtest(){
	#自动选择下载节点
	#原来已失效:libAddr='https://download.libsodium.org/libsodium/releases/libsodium-1.0.13.tar.gz'
...
install_centos_ssr(){
	...
	ldconfig
	git clone -b manyuser https://github.com/williamsleed/shadowsocks.git "/root/shadowsocks"
	cd /root/shadowsocks
...
install_node(){
	...
	echo "# Github: https://github.com/williamsleed/ss-panel-and-ss-py-mu          #"
	echo "# Author: 91vps                                                          #"
	echo "##########################################################################"
	echo
	#Check Root
